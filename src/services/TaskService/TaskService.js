...
const TaskService = {
  ...
  getRequestByRequestId: async (requestId) => {
    try {
      ...
      // Log lỗi để debug
      console.error(
        "Error fetching task by request ID:",
        error.response ? error.response.data : error.message
      );
    }
  },
  GetContractByRequestCharacterId: async (requestCharacterId) => {
    try {
      const response = await apiClient.get(
        `/api/Contract/GetContractByRequestCharacterId?requestCharacterId=${requestCharacterId}`
      );
      return response.data; // Trả về dữ liệu từ API
    } catch (error) {
      // Log lỗi để debug
