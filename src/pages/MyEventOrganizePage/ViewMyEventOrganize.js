...
const ViewMyEventOrganize = ({ requestId }) => {
  const [requestData, setRequestData] = useState(null);
  const [packageData, setPackageData] = useState(null); // New state for package data
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [expandedCharacters, setExpandedCharacters] = useState({});
  const [expandedDates, setExpandedDates] = useState({});
  ...
  useEffect(() => {
    const fetchRequestAndPackage = async () => {
      ...
      try {
        ...
        setError(error.message || "Failed to fetch request or package details");
        toast.error(
          error.message || "Failed to fetch request or package details"
        );
      } finally {
        setLoading(false);
      }
