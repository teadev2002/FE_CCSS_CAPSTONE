...
//       const existingCosplayerIds = requests
...
//         dates: dates,
//         accountId:
//           selectedCosplayers.length > 0 ? selectedCosplayers.join(",") : "",
//       };
...
//       const startDate = dateRange[0];
//       const maxEndDate = startDate.add(14, "day");
...
//       toast.error("The date range cannot exceed 14 days!");
//       setDateRange(null);
//       return;
//     }
...
//     }
//     if (diffDays > 14) {
//       toast.error("Rental period cannot exceed 14 days!");
//       return;
//     }
...
//     const today = dayjs().startOf("day");
//     const minStartDate = today.add(4, "day"); // 19/05/2025
//     const maxStartDate = minStartDate.add(14, "day"); // 02/06/2025

//     // Disable dates before 19/05/2025
//     if (current < minStartDate) {
//       return true;
//     }

//     // If selecting startDate (no startDate selected yet)
//     if (!dateRange || !dateRange[0]) {
//       // Disable dates after 02/06/2025 for startDate
//       return current > maxStartDate;
//     }

//     // If selecting endDate (startDate is selected)
//     const startDate = dateRange[0];
//     const maxEndDate = startDate.add(14, "day"); // Max endDate is startDate + 14 days

//     // Disable dates before startDate or after startDate + 14 days
//     return current < startDate || current > maxEndDate;
//   };
...
//     // Validate startDate is at least 4 days from today
//     if (start.isBefore(minStartDate, "day")) {
//       toast.error("Start date must be at least 4 days from today!");
//       setDateRange(null);
//       return;
...
//     // Validate startDate is not more than 14 days from minStartDate
...
//     // Validate endDate is not more than 14 days from startDate
//     const maxEndDate = start.add(14, "day");
...
//     // Validate rental period is between 1 and 5 days (inclusive)
...
//     // Validate start date is at least 4 days from today
//     if (startDate.isBefore(minStartDate, "day")) {
//       toast.error("Start date must be at least 4 days from today!");
//       return;
//     }

//     // Validate start date is not more than 14 days from minStartDate
//     if (startDate.isAfter(maxStartDate, "day")) {
//       toast.error("Start date cannot be more than 14 days from 19/05/2025!");
//       return;
//     }

//     // Validate end date is not before startDate
//     if (endDate.isBefore(startDate, "day")) {
//       toast.error("End date must be on or after start date!");
//       return;
//     }

//     // Validate end date is not more than 14 days from startDate
//     const maxEndDate = startDate.add(14, "day");
...
const CosplayersPage = () => {
  ...
  // Fetch initial data (characters and user info)
  useEffect(() => {
    const accessToken = localStorage.getItem("accessToken");
    if (!accessToken) {
  ...
  // Fetch cosplayers based on character and time selection
  const fetchCosplayersByCharacterAndTime = async (characterId) => {
    if (!dateRange || Object.keys(timeRanges).length === 0 || !characterId) {
      setFilteredCosplayers([]);
      setCosplayers([]);
  ...
  // Calculate total hours for pricing
  ...
  // Precompute cosplayer prices when modal is opened
  useEffect(() => {
  ...
  // Handle individual cosplayer selection
  const handleCosplayerSelect = (cosplayerId) => {
  ...
  // Add selected cosplayers to request
  const handleAddCosplayers = () => {
  ...
  // Prepare request for confirmation
  const handleSendRequest = () => {
    if (!dateRange || Object.keys(timeRanges).length === 0 || !location) {
      toast.error(
  ...
  // Remove a cosplayer from the request
  const handleRemoveCosplayer = (cosplayerId, characterId) => {
    setModalData((prev) => ({
      ...prev,
      listRequestCharacters: prev.listRequestCharacters.filter(
  ...
  const handleModalConfirm = async () => {
    ...
    // Validate timeRanges for all days
