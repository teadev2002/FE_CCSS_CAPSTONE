...
const CosplayersPage = () => {
  ...
  const fetchCosplayersByCharacterAndTime = async (characterId) => {
    ...
    setLoading(true);
    try {
      // Chuẩn bị dữ liệu cho API ChangeCosplayer
      const dates = [];
      const startDate = dateRange[0];
      const endDate = dateRange[1];
      const days = endDate.diff(startDate, "day") + 1;

      // Tạo danh sách dates từ timeRanges
      for (let i = 0; i < days; i++) {
        const day = startDate.add(i, "day").format(dateFormat);
        const [startTime, endTime] = timeRanges[day] || [];
        if (startTime && endTime) {
      ...
      // Lấy characterId từ character được chọn
      const character = characters.find((char) => char.id === characterId);
      if (!character) {
        throw new Error("Character not found");
      }

      // Lấy danh sách cosplayer đã chọn từ requests để loại bỏ
      ...
      // Lọc cosplayer dựa trên yêu cầu chiều cao, cân nặng và loại bỏ cosplayer đã chọn
      const filtered = mappedCosplayers.filter(
        (cosplayer) =>
