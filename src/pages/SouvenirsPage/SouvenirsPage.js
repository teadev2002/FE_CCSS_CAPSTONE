    ...
    <div className="costume-rental-page min-vh-100">
        ...
        <div className="costume-grid">
          {paginatedSouvenirs.map((souvenir) => (
            <div className="costume-card" key={souvenir.id}>
              <div className="card-image">
                <img
                  src={souvenir.image || "https://via.placeholder.com/300"}
                  alt={souvenir.name}
                  className="img-fluid"
                />
              </div>
      ...
      >
        ...
        <Modal.Body>
          {selectedSouvenir && (
            <div className="costume-gallery">
              {/* Hiển thị tất cả hình ảnh trong Carousel */}
              <Carousel className="gallery-carousel">
                {selectedSouvenir.productImages && selectedSouvenir.productImages.length > 0 ? (
                  selectedSouvenir.productImages.map((image, index) => (
                    <Carousel.Item key={index}>
                      <div className="carousel-image-container">
                        <img
                          className="d-block w-100"
                          src={image.urlImage || "https://via.placeholder.com/300"}
                          alt={`${selectedSouvenir.name} - Image ${index + 1}`}
                        />
                      </div>
                      <Carousel.Caption>
                        <h3>{selectedSouvenir.name}</h3>
                        <p>Image {index + 1} of {selectedSouvenir.productImages.length}</p>
                      </Carousel.Caption>
                    </Carousel.Item>
                  ))
                ) : (
