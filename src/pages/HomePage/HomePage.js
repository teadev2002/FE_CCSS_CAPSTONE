...
import React, { useEffect, useState, useCallback } from "react";
import { Link, useNavigate } from "react-router-dom";
import { Carousel, Row, Col, Form, Button, Modal } from "react-bootstrap";
import {
  Shirt,
  CalendarDays,
  Users,
...
import "bootstrap/dist/css/bootstrap.min.css";
import "../../styles/HomePage.scss";
import CharacterService from "../../services/HomePageService/CharacterService.js";
import CosplayerService from "../../services/HomePageService/CosplayerService.js";
...
const HomePage = () => {
  ...
  // Handle "Rent Costumes" button
  const handleRentCostumes = () => {
    navigate("/costumes");
  };
  ...
  // State for Character Modal
  const [showCharacterModal, setShowCharacterModal] = useState(false);
  const [selectedCharacter, setSelectedCharacter] = useState(null);

  // State for Cosplayer List
  const [cosplayers, setCosplayers] = useState([]);
  const [filteredCosplayers, setFilteredCosplayers] = useState([]);
  const [currentPageCosplayers, setCurrentPageCosplayers] = useState(1);
  const [
  ...
  // State for About Us image carousel
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  const aboutUsImages = [
    "https://c4.wallpaperflare.com/wallpaper/721/4/835/cosplay-nier-automata-2b-no-2-yorha-wallpaper-preview.jpg",
    "https://a-static.besthdwallpaper.com/war-devil-chainsaw-man-cosplay-wallpaper-3554x1999-106179_53.jpg",
  ...
  // Format hourly rate for cosplayers
  const formatHourlyRate = (rate) => {
    if (rate == null) {
      return "N/A";
    }
  ...
  // Handle opening character modal
  const handleCharacterShow = async (character) => {
    try {
      const characterData = await CharacterService.getCharacterById(
  ...
  // Handle closing character modal
  ...
  // Fetch images for current characters and cosplayers
  useEffect(() => {
    currentCharacters.forEach((character) => {
      fetchCharacterImage(character.characterId);
    });
  ...
  // Auto-scroll for Character List
  useEffect(() => {
    const intervalTime = isPageManuallySelectedCharacters ? 30000 : 8000;
    const timer = setInterval(() => {
      setCurrentPageCharacters((prev) => {
  ...
  // Auto-scroll for Cosplayer List
  useEffect(() => {
    const intervalTime = isPageManuallySelectedCosplayers ? 30000 : 8000;
    const timer = setInterval(() => {
      setCurrentPageCosplayers((prev) => {
  ...
  // Auto-scroll for About Us images
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentImageIndex((prev) => (prev + 1) % aboutUsImages.length);
    }, 7000);
  ...
  // Handle character search and filter
  const handleCharacterSearch = () => {
    const filtered = characters.filter((char) => {
      const nameMatch = characterSearchParams.characterName
        ? char.characterName
  ...
  // Handle cosplayer search and filter
  const handleCosplayerSearch = () => {
    const filtered = cosplayers.filter((cosplayer) => {
      const nameMatch = cosplayerSearchParams.cosplayerName
        ? cosplayer.name
      ...
      const rateMatch =
        cosplayer.salaryIndex >= cosplayerSearchParams.hourlyRate[0] &&
        cosplayer.salaryIndex <= cosplayerSearchParams.hourlyRate[1];
      const isActiveMatch = cosplayer.isActive === true;
      return (
        nameMatch &&
        starMatch &&
        heightMatch &&
  ...
  // Handle cosplayer input change
  const handleCosplayerInputChange = (e) => {
    const { name, value } = e.target;
    setCosplayerSearchParams((prev) => ({ ...prev, [name]: value }));
  };

  // Handle character range change
  const handleCharacterRangeChange = (name, values) => {
    setCharacterSearchParams((prev) => ({ ...prev, [name]: values }));
  };

  // Handle cosplayer range change
  const handleCosplayerRangeChange = (name, values) => {
    setCosplayerSearchParams((prev) => ({ ...prev, [name]: values }));
  };
  ...
  // Calculate image indices for About Us carousel
  const getImageIndices = () => {
    const length = aboutUsImages.length;
    const prevIndex = (currentImageIndex - 1 + length) % length;
    const nextIndex = (currentImageIndex + 1) % length;
  ...
  return (
    <div className="homepage">
      ...
      <div className="custom-section featured-characters py-5">
                  ...
                  <div className="card-inner">
                    <div className="card-front">
                      ...
                      <Link
                        ...
                        onClick={(e) => {
                          e.preventDefault();
                          handleCharacterShow(character);
                        }}
                        aria-label={`View details of ${character.characterName}`}
                      >
                        <Eye size={20} />
                      </Link>
                      <div className="card-content">
        ...
        <div className="text-center mt-4">
          <button className="view-all-button" onClick={handleRentCostumes}>
            Rent Costumes
          </button>
        </div>
      </div>
      ...
      <Modal
        ...
        onHide={handleCharacterClose}
        size="lg"
        centered
        className="gallery-modal"
      >
        <Modal.Header closeButton>
          <Modal.Title>{selectedCharacter?.characterName} Gallery</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedCharacter && (
            <div className="costume-gallery">
                  ...
                  <Carousel.Item key={index}>
                    <div className="carousel-image-container">
                      <img
                        className="d-block w-100"
                        src={image.urlImage}
                        alt={`${selectedCharacter.characterName} - Image ${index + 1
                          }`}
                      />
                    </div>
                    <Carousel.Caption>
                      <h3>{selectedCharacter.characterName}</h3>
                      <p>{`Image ${index + 1} of ${selectedCharacter.images.length
                        }`}</p>
                    </Carousel.Caption>
                  </Carousel.Item>
                ))}
              ...
              <div className="costume-details mt-4">
                <h4>About {selectedCharacter.characterName}</h4>
                <p>{selectedCharacter.description}</p>
                <div className="costume-info">
                  <div className="info-item">
                    <strong>Height:</strong>{" "}
                    {`${selectedCharacter.minHeight} - ${selectedCharacter.maxHeight} cm`}
                  </div>
                  <div className="info-item">
                    <strong>Weight:</strong>{" "}
                    {`${selectedCharacter.minWeight} - ${selectedCharacter.maxWeight} kg`}
                  </div>
                  <div className="info-item">
                    <strong>Price:</strong>{" "}
                    {selectedCharacter.price.toLocaleString()} VND
                  </div>
                  <div className="info-item">
                    <strong>Available Quantity:</strong>{" "}
                    {selectedCharacter.quantity}
                    {selectedCharacter.quantity <= 3 &&
                      selectedCharacter.quantity > 0
                      ? ` (Only ${selectedCharacter.quantity} left!)`
                      : ""}
                  </div>
                </div>
