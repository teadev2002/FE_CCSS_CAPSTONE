...
//               <Dropdown>
//                 <Dropdown.Toggle className="sort-dropdown" id="ticket-sort-dropdown">
//                   Sort: {ticketSortOrder === "newest" ? "Nearest Upcoming" : "Distant Upcoming"}
//                 </Dropdown.Toggle>
//                 <Dropdown.Menu>
...
import { Container, Row, Col, Card, Badge, Dropdown } from "react-bootstrap";
import { Pagination, Tabs, Spin } from "antd";
import { DollarSign, Ticket, ShoppingBag, Calendar, CreditCard, XCircle, RefreshCw } from "lucide-react";
import "../../styles/PurchaseHistory.scss";
import PurchaseHistoryService from "../../services/PurchaseHistoryService/PurchaseHistoryService";
import { jwtDecode } from "jwt-decode";
import Box from "@mui/material/Box";
...
const PurchaseHistory = () => {
  ...
  const formatDateTime = (dateString) => {
    if (!dateString) return "N/A";
    let date;
    // Handle transaction format: "HH:mm dd/MM/yyyy"
    const transactionRegex = /^(\d{2}):(\d{2}) (\d{2})\/(\d{2})\/(\d{4})$/;
    if (transactionRegex.test(dateString)) {
      const [, hours, minutes, day, month, year] = dateString.match(transactionRegex);
      date = new Date(`${year}-${month}-${day}T${hours}:${minutes}:00`);
    } else {
      // Handle ISO format for tickets: "yyyy-MM-ddTHH:mm:ss"
      date = new Date(dateString);
    }
    if (isNaN(date.getTime())) return "Invalid Date";
  ...
  useEffect(() => {
    const fetchPurchaseHistory = async () => {
      ...
      try {
        ...
        setFestivalTickets(
          [...ticketsData].sort((a, b) => {
            const dateA = parseDateTime(a.ticket.event.startDate);
            const dateB = parseDateTime(b.ticket.event.startDate);
            return ticketSortOrder === "newest" ? dateA - dateB : dateB - dateA; // Nearest Upcoming = earliest startDate
          })
        );
        setPurchasedOrders(
          [...ordersData].sort((a, b) => {
        ...
        setTransactions(
          [...allTransactions]
            ...
            .sort((a, b) => {
              const dateA = parseDateTime(a.creatAt);
              const dateB = parseDateTime(b.creatAt);
              return transactionSortOrder === "newest" ? dateB - dateA : dateA - dateB; // Newest = latest creatAt
            })
        );
      } catch (error) {
        console.error("Failed to fetch purchase history:", error);
        ...
        <Tabs defaultActiveKey="2" type="card">
          <TabPane tab="Festival Tickets" key="1">
            <div className="sort-container mb-3">
              <Dropdown>
                <Dropdown.Toggle className="sort-dropdown" id="ticket-sort-dropdown">
                  Sort: {ticketSortOrder === "newest" ? "Nearest Upcoming" : "Furthest Upcoming"}
                </Dropdown.Toggle>
                <Dropdown.Menu>
                  <Dropdown.Item onClick={() => setTicketSortOrder("newest")}>Nearest Upcoming</Dropdown.Item>
                  <Dropdown.Item onClick={() => setTicketSortOrder("oldest")}>Furthest Upcoming</Dropdown.Item>
                </Dropdown.Menu>
              </Dropdown>
            </div>
            {loading ? (
                        ...
                        <Card.Body>
                          <div className="d-flex flex-column gap-4">
                            ...
                            <div className="event-details">
                              ...
                              <div className="text-muted small">
                                <Calendar size={16} /> <strong>Start Date:</strong>{" "}
                                {formatDateTime(ticket.ticket.event.startDate)}
                              </div>
                              <div className="text-muted small">
                                <Calendar size={16} /> <strong>End Date:</strong>{" "}
                                {formatDateTime(ticket.ticket.event.endDate)}
                              </div>
                            </div>
                          </div>
                        </Card.Body>
                        ...
                        <Card.Body>
                          <div className="d-flex flex-column gap-4">
                            ...
                            <div className="transaction-details">
                              ...
                              <div className="text-muted small">
                                <Calendar size={16} /> <strong>Transaction Date:</strong>{" "}
                                {formatDateTime(transaction.creatAt)}
                              </div>
                            </div>
                          </div>
                        </Card.Body>
