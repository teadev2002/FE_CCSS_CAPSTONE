...
const ViewMyRentalCostume = ({
      ...
      getRequestByRequestId(requestId)
        .then(async (data) => {
          ...
          try {
            ...
            if (!accountId)
              throw new Error("Account ID not found in request data");
            ...
            if (matchingContract && matchingContract.contractId) {
              const contractResponse =
                await MyRentalCostumeService.getContractByContractId(
  ...
  const handleCancelRequest = async () => {
    ...
    setLoading(true);
    try {
      await MyRentalCostumeService.UpdateRequestStatusById(
        requestId,
        2, // Status code for "Cancel"
        cancelReason
      );
      setRequestData((prev) => ({
        ...prev,
