...
//     }
//   };
//   const isTaskDateValid = (startDate) => {
//     const now = new Date();
//     const parsedStart = parseDateTime(startDate);
//     if (!parsedStart.date || parsedStart.date === "N/A") {
//       console.warn(`Invalid startDate: ${startDate}`);
//       return false;
//     }

//     const [day, month, year] = parsedStart.date.split("/").map(Number);
...
//     // Check if current date matches startDate
//     return currentDate.getTime() === startDateObj.getTime();
//   };
//   useEffect(() => {
...
//     // Parse startDate
//     const parsedStart = parseDateTime(selectedTask.startDate);
//     if (!parsedStart.date || parsedStart.date === "N/A") {
//       toast.warn("Invalid task start date.", {
//         position: "top-right",
//         autoClose: 3000,
//       });
//       return;
...
//       toast.warn("Task date not coming yet.", {
//         position: "top-right",
//         autoClose: 3000,
//       });
//       return;
...
//                     !isTaskDateValid(selectedTask?.startDate)
//                   }
//                 >
//                   {isLoading ? (
//                     <>
...
const MyTask = () => {
  ...
  const handleUpdateStatus = async () => {
    ...
    setIsLoading(true);
    try {
      // Fetch the latest task data to check isValidate
      const taskData = await TaskService.getTaskById(selectedTask.taskId);
      ...
      // Proceed with status update if isValidate is false or undefined
      await TaskService.updateTaskStatus(
        selectedTask.taskId,
        selectedStatus,
        id
