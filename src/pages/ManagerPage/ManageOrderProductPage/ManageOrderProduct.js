...
const ManageOrderProduct = () => {
  ...
  const [sortOrder, setSortOrder] = useState({
    field: "orderDate",
    order: "desc", // Default to newest orders first
  });
  const [currentPage, setCurrentPage] = useState(1);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const rowsPerPageOptions = [10, 20, 30];
  ...
  const filterAndSortData = (data, search, sort) => {
    ...
    return filtered.sort((a, b) => {
      if (sort.field === "totalPrice") {
        // Numeric sort for totalPrice
        return sort.order === "asc"
          ? a.totalPrice - b.totalPrice
          : b.totalPrice - a.totalPrice;
      } else if (sort.field === "shipStatus") {
        // Logical status order: Waiting for Confirmation (0), Waiting to Pick (1), In Transit (2), Received (3), Cancel (4), Refund (5)
        const statusOrder = {
          "Waiting for Confirmation": 0,
          "Waiting to Pick": 1,
          "In Transit": 2,
        ...
        return sort.order === "asc"
          ? statusOrder[a.shipStatus] - statusOrder[b.shipStatus]
          : statusOrder[b.shipStatus] - statusOrder[a.shipStatus];
      } else if (sort.field === "orderDate") {
        // Date sort for orderDate
        return sort.order === "asc"
          ? new Date(a.orderDate) - new Date(b.orderDate)
          : new Date(b.orderDate) - new Date(a.orderDate);
      } else {
        // Default string sort for other fields (though only totalQuantity is left)
        const valueA = String(a[sort.field] || "").toLowerCase();
        const valueB = String(b[sort.field] || "").toLowerCase();
        return sort.order === "asc"
          ? valueA.localeCompare(valueB)
