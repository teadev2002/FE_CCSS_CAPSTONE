...
//     try {
//       // const btnEdit = dayjs();
//       // const createdDate = dayjs(item.createdDate, "HH:mm DD/MM/YYYY");

//       // if (btnEdit.diff(createdDate, "day") < 1) {
//       //   toast.info("Browsed is required 1 day after created Date.", {
//       //     position: "top-right",
//       //     autoClose: 2000,
//       //   });
//       //   return;
...
//       setShowModal(true);
//     } catch (error) {
//       // Handle errors but still respect the date check
//       setIsEditing(true);
//       setCurrentItem(item);
//       setFormData({ status: item.statusRequest, reason: "" });
//       setShowModal(true);
...
const ManageRequest = () => {
    ...
    try {
      ...
      const formattedData = data.map((req) => {
        ...
        return {
          ...
          endDate,
          reason: req.reason || "",
          isValidate: req.isValidate || false, // Add isValidate
        };
      });
      setRequests(formattedData);
      setLoading(false);
  ...
  const handleShowModal = async (item) => {
    try {
      ...
      console.error("Error in handleShowModal:", error.message);
      toast.error(error.message || "Failed to process request.", {
        position: "top-right",
        autoClose: 3000,
      });
      // Only open modal if isValidate is false or undefined
      if (item.isValidate !== true) {
        setIsEditing(true);
        setCurrentItem(item);
        setFormData({ status: item.statusRequest, reason: "" });
