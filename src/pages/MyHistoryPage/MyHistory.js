...
const MyHistory = () => {
  ...
  const handleCompletePaymentConfirm = async () => {
    ...
    try {
      ...
      // Giả lập thanh toán thành công (thực tế cần callback từ payment gateway)
      await MyHistoryService.updateStatusContract(
        completePaymentData.contractId,
        "Completed"
      );
      const contractData = await MyHistoryService.getAllContractByAccountId(
        accountId
...
//         (request) =>
//           request.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
//           (dayjs(request.startDate).isValid() &&
//             dayjs(request.startDate).format("DD/MM/YYYY").includes(searchTerm))
//       );
//     setFilteredPendingRequests(filtered);
//     setCurrentPendingPage(1);
//   }, [
...
//           .toLowerCase()
//           .includes(searchTerm.toLowerCase()) ||
//         (dayjs(contract.startDate).isValid() &&
