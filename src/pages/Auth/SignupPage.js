...
//                 {/* Tạm ẩn tính năng đăng ký bằng Google vì chưa hỗ trợ */}
//                 {/* <div className="separator">
//                   <span>OR</span>
//                 </div>
...
const SignupPage = () => {
  ...
  // State để điều khiển hiển thị modal xác nhận đăng ký
  const [showModal, setShowModal] = useState(false);
  // State để theo dõi trạng thái đã gửi form đăng ký thành công
  ...
  const handleSubmit = async (e) => {
    ...
    // Nếu đã gửi form thành công, mở lại modal xác minh
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        ...
        if (response === "Email existed!!!") {
          ...
          setShowModal(true);
          setIsSignupSubmitted(true); // Đánh dấu đã gửi form thành công
          toast.info("Please check your email for a verification code.", {
            position: "top-right",
            autoClose: 5000,
            hideProgressBar: false,
  ...
  // Hàm xử lý khi xác minh thành công
  const handleVerificationSuccess = () => {
    setShowModal(false);
    setIsSignupSubmitted(false); // Reset trạng thái sau khi xác minh
  ...
  return (
    <div className="gradient-bg min-vh-100">
      ...
      <ModalConfirmSignUp
        ...
        onHide={() => setShowModal(false)}
        email={formData.email}
        onSuccess={handleVerificationSuccess} // Truyền callback khi xác minh thành công
      />
