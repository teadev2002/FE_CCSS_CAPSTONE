  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        console.error("Failed while decoding token:", decodeError);
      }
      ...
      if (userRole === "Cosplayer" && decoded) {
        ...
        navigate(`/manage/contract`);
      }else if (userRole === "Admin" && decoded) {
      ...
      setError(err.message);
      toast.error(
        "Login failed: " + (err.message || "Something went wrong")
      );
      console.error("Login error:", err);
    } finally {
      setLoading(false);
          ...
          <Col md={6}>
            <div className="login-form">
              ...
              <Form onSubmit={handleSubmit}>
                ...
                {/* Tạm ẩn tính năng đăng nhập bằng Google vì chưa hỗ trợ */}
                {/* <div className="separator">
                  <span>OR</span>
                </div>
